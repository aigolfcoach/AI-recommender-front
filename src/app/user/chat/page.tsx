      ...
      <div className="layout-container flex h-full grow flex-col">
          ...
          <div className="flex flex-1 justify-end gap-8">
            <div className="flex items-center gap-9">
              ...
              <button 
                ...
                className="text-[#101518] text-sm font-medium leading-normal hover:text-[#9cc0de]"
              >
        ...
        <div className="px-40 flex flex-1 justify-center py-5">
          <div className="layout-content-container flex flex-col max-w-[960px] flex-1">
            ...
            {showHistory && (
              <div className="p-4">
                <h3 className="text-[#101518] text-lg font-bold leading-tight tracking-[-0.015em] pb-2">Recent Conversations</h3>
                {conversations.length > 0 ? (
                  <div className="space-y-3">
                    {conversations.map((conversation) => (
                      <div key={conversation.id} className="bg-white border border-[#d4dce2] rounded-lg p-4 hover:bg-gray-50 transition-colors">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center space-x-2">
                            {conversation.responses
                              ...
                              .map(r => (
                                <span key={r.providerName} className="text-xs bg-[#eaeef1] px-2 py-1 rounded">
                                  {r.providerName}
                                </span>
                              ))}
                          </div>
                        </div>
                        <p className="text-[#5c758a] text-sm mb-2">{conversation.question}</p>
                        <div className="text-xs text-[#5c758a]">
                          Temperature: {conversation.temperature} | Max Tokens: {conversation.maxTokens}
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-[#5c758a] text-sm">No conversations yet. Start chatting!</p>
                )}
              </div>
            )}
